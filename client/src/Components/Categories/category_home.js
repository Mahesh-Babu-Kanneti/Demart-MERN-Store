...
const homeProduct = async()=>{
    let response = await axios.get(`${process.env.REACT_APP_BASE_URL}/products/list`).then(
        res=>{setLoad(false);setHomeproducts(res.data)}
    )
...
useEffect(()=>{
            ...
            <Toaster
                ...
                <div style={{ width: "auto" }}>
                    ...
                    <Card style={{ border: "none", marginTop: "-15px", borderRadius: "0px" }}>
                        <Card.Body style={{ margin: "10px 0px 0px 15px" }}> <h5> Home & Hygiene </h5>  </Card.Body>
                    </Card>
 ...
 {load ? <Loader/> : <>
                    ...
                    {homeProducts.filter(li=>li.productCategory==='home_hygiene').map(list => <div style={{ display: "inline-flex" }}  key={list._id}>
